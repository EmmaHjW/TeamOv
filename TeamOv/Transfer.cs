    ...
    public class Transfer
    {
        ...
        public void Deposit(string loggedInCustomer) //Deposit to accounts
        {
            ...
            while (input == "Y" || input == "y")
            {
                ...
                else
                {
                    ...
                    Console.ResetColor();
                    Transactionservice.transactionslist.Add($"{DateTime.Now} Deposit: {amount} {ToAccount.Currency} to Account number: {ToAccount.AccountNumber} Account owner: {ToAccount.Owner}");
                    Console.WriteLine();
                }
                Console.ForegroundColor = ConsoleColor.Yellow;
        ...
        public void Withdraw(string loggedInCustomer)
        {
            ...
            while (input == "Y" || input == "y")
            {
                ...
                else
                {
                    ...
                    Console.ResetColor();
                    Transactionservice.transactionslist.Add($"{DateTime.Now} Withdraw: {amount} {FromAccount.Currency} from account number: {FromAccount.AccountNumber} Account owner: {FromAccount.Owner}");
                    Console.WriteLine();
                }
                Console.WriteLine();
                Console.ForegroundColor = ConsoleColor.Yellow;
                ...
                while (decimal.TryParse(Console.ReadLine(), out amount)) //Check that amount is valid to transfer
                {
                    ...
                    else
                    {
                        ...
                        if (FromAccount.Currency == "SEK")
                        {
                            ...
                            Console.ResetColor();
                            Transactionservice.transactionslist.Add($"{DateTime.Now} {FromAccount.AccountNumber} Transfer added {tempSekAmount.ToString("N" + 2)} {FromAccount.Currency}\n" +
                        ...
                        else if (ToAccount.Currency == "SEK")
                        {
                            ...
                            Console.ResetColor();
                            Transactionservice.transactionslist.Add($"{DateTime.Now} {FromAccount.AccountNumber} Transfer added {amount.ToString("N" + 2)} {FromAccount.Currency}\n" +
                                $"{DateTime.Now.AddMinutes(15)} {ToAccount.AccountNumber} Transfer received {tempSekAmount} {ToAccount.Currency}");
                        }
                        else
                        {
                            FromAccount.Balance -= amount;
        ...
        public void ThirdPartTransfer(string loggedInCustomer)
        {
            ...
            while (input == "Y" || input == "y")
            {
                ...
                {
                    Console.WriteLine($"Account ID: {owner.AccountId} Account number: {owner.AccountNumber} Account name: {owner.AccountName} Balance: {owner.Balance} {owner.Currency} Owner: {owner.Owner} ");
                }
                ...
                while (decimal.TryParse(Console.ReadLine(), out amount)) //Check that amount is valid to transfer
                {
                    ...
                    else
                    {
                        ...
                        Console.WriteLine($"New balance: {FromAccount.Balance}.SEK left on your {FromAccount.AccountName}");
                        Console.ResetColor();
                        Transactionservice.transactionslist.Add($"{DateTime.Now} Transfer added {amount.ToString("N" + 2)} {FromAccount.Currency} Account: {FromAccount.AccountNumber} Account owner: {FromAccount.Owner}\n" +
                            $"{DateTime.Now.AddMinutes(15)} Transfer received {amount.ToString("N" + 2)} {ToAccount.Currency} Account: {ToAccount.AccountNumber} Account owner: {ToAccount.Owner} ");
                    }
                    Console.ResetColor();
                    Console.WriteLine();
                    Console.ForegroundColor = ConsoleColor.Yellow;
